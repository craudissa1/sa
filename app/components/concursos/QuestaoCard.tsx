...
import { Questao, AlternativaQuestao as Alternativa } from "@/app/stores/questoesStore"; // Ajustado para importar AlternativaQuestao como Alternativa
...
export function QuestaoCard({ questao, modo, onResponder, numeroQuestao }: QuestaoCardProps) {
  const [respostaSelecionada, setRespostaSelecionada] = useState<string | undefined>(
    modo === "revisao" && questao.respostaUsuario ? questao.respostaUsuario : undefined
  );
  ...
  const handleResposta = (valor: string) => {
    ...
    if (modo === "simulado" && onResponder && questao.id) {
      const alternativaCorreta = questao.alternativas.find(alt => alt.id === questao.resposta_correta_id);
      const acertou = valor === questao.resposta_correta_id;
  ...
  const getStatusAlternativa = (alt: Alternativa): "correta" | "incorreta" | "neutra" => {
    ...
    if (alt.id === questao.resposta_correta_id) return "correta"; // Checa contra o ID da resposta correta da questão
    if (respostaSelecionada === alt.id && alt.id !== questao.resposta_correta_id) return "incorreta";
    return "neutra";
  };
      ...
      <div className={` ${interfaceSimplificada ? "space-y-1" : "space-y-3"}`}>
        ...
        <div className="space-y-2">
          {questao.alternativas.map((alt) => {
            ...
            const inputId = `q-${questao.id || crypto.randomUUID()}-alt-${alt.id}`;
            ...
            return (
              ...
              >
                <input
                  type="radio"
                  id={inputId}
                  name={`questao-${questao.id || crypto.randomUUID()}`}
                  value={alt.id}
        ...
        {modo === "estudo" && !mostrarJustificativa && respostaSelecionada && questao.id && (

...
// Função auxiliar para resetar dados de mock entre testes
function resetMockData() {
  // Preserva apenas os usuários e reseta todas as outras coleções
...
const testUser = {
  ...
  password: 'senha123',
  nome_completo: 'Usuário Teste E2E' // Adicionada propriedade nome_completo para consistência
};
...
describe('Testes E2E StayFocus com Supabase', () => {
  ...
  beforeEach(async () => {
    // Resetar dados de mock para garantir isolamento entre testes
    resetMockData();
    
    // Limpar estado anterior
    await supabase.auth.signOut();
    
    // Simular limpeza de storage no navegador
    ...
    // Certificar que o usuário está logado antes de prosseguir
  ...
  afterEach(async () => {
    // Com o mock Supabase, o cleanup é mais simples - basta resetar os dados
    resetMockData();
    
    // Resetar IDs de cleanup para evitar tentativas de deleção de dados que não existem mais
    cleanupIds = {
      tarefa: null,
      medicamento: null,
      prioridade: null,
    ...
    // Verificar que o logout foi efetivo
    const { data: userData } = await supabase.auth.getUser();
